...
EXEC=$(basename "$0")
USAGE="Usage: $EXEC [-h] [-m MEMORY_GB] [-c CASSANDRA_HOST] [-s SPARK_MASTER] [--src_keyspace RAW_KEYSPACE] [--tag_keyspace TAG_KEYSPACE] [--tgt_keyspace TGT_KEYSPACE] [--bucket_size BUCKET_SIZE]"
...
"$SPARK_HOME"/bin/spark-submit \
  ...
  target/scala-2.12/graphsense-ethereum-transformation_2.12-0.5.2-SNAPSHOT.jar \
  --raw-keyspace "$RAW_KEYSPACE" \
  --tag-keyspace "$TAG_KEYSPACE" \
  --target-keyspace "$TGT_KEYSPACE" \
  --bucket-size "$BUCKET_SIZE"
